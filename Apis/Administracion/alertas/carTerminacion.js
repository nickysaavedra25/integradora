...
if(!token){
    ...
    fetch(apiUrl + "/api/cartaTer/contarCartasTer", {
    ...
    .then(data => {
        // Verificar que la respuesta contenga la propiedad 'total_cartasTer'
        if (data.total_cartasTer !== undefined) {
            // Mostrar el total de cartas de terminación
            document.getElementById("total_cartas").textContent = data.total_cartasTer;


            // Mostrar la lista de nombres de alumnos
            const lista = document.getElementById("listaCartasTerLista");
            ...
            alert("Error: " + (data.mensaje || "Error al obtener las cartas de Terminación "));
        }
    })
    .catch(error => {
